...
var ConstellationContext = function ()
{
    ...
    this.loadConfig = function(url, accesskey, applicationName, sdkVersion) {
        ...
        if(arguments.length === 0 || !url || !accesskey || !applicationName || !sdkVersion)
        {
            //// load config file
            that.check();
    ...
    this.createHub = function (type, params) {
        var $ = self.$ = window.$;
        self.hub = self.hub || $.hubConnection['createConstellation' + type](params.url, params.appName, params.accessKey);
        self[type.toLowerCase() + 'Hub'] = self.hub;
        return self;
    };

    this.getPackageHub = function(sentinelName) {
        //return this.createHub(HubType.PACKAGE, {url : url, sentinelName : sentinelName, packageName : appName, accessKey : accessKey });
        var $ = self.$ = window.$;
        self.hub = self.hub || $.signalR['createConstellationPackage'](self.url, sentinelName, self.applicationName, self.accessKey);
        self['packageHub'] = self.hub;
        return self;
    };

    this.getControllerHub = function() {
        return this.createHub(HubType.CONTROLLER, { url : self.url, accessKey : self.accessKey, appName : self.applicationName });
    };

    this.getConsumerHub = function() {
        return this.createHub(HubType.CONSUMER, { url : self.url, accessKey : self.accessKey, appName : self.applicationName });
    };
